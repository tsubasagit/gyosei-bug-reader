...
- **吹き出しの英語化（第1話）**：`tools/letter_en.py` がコマの絵の吹き出しの日本語を白で消し、`script.en.json` の英語を書き入れる（絵そのものは変えない）。出力は `ep/<話ID>/pages-en/`、全ページそろうと episodes.json の `pagesEn` が true になり、英語で読むときは本編がこの絵になる（扉絵は日本語のまま。対訳の列は出さない）

  ```
  python tools/letter_en.py "<話フォルダ>" <話ID> --out <作業フォルダ> --candidates   # 吹き出しの候補に番号を振った確認画像
  python tools/letter_en.py "<話フォルダ>" <話ID> --out <作業フォルダ> --publish      # pages-en を書き出す
  ```

  自動の割り当ては外れやすいので、`ep/<話ID>/letter.en.json` でセリフごとに吹き出しを指定する（候補の番号・吹き出しの中の点・囲む四角・`skip`・2つの吹き出しに分ける `parts`）。描き文字（カタカナだけのもの）と、表で `skip` にした図の中の文字は日本語のまま

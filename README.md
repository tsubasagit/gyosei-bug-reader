...
- 1ページずつ表示する。PC など幅のある画面では、ページを横幅に合わせて（最大 960px）大きく置き、縦にスクロールして読む。スペースキーはスクロールして、ページの端まで来たらめくる
- 見開き（右が若いページ）はボタンで選べる
- マウスで読む画面では、上下のバーは画面の上端か下端にマウスを持っていくと出る
